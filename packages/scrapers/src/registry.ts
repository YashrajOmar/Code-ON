...
import type {
  ...
  ProblemScraperOptions,
} from './types';

import { CodeforcesSubmissionScraper } from './platforms/codeforces-submissions';
import { LeetCodeSubmissionScraper } from './platforms/leetcode-submissions';
import { LeetCodeProblemScraper } from './platforms/leetcode-problems';
import { CodeforcesProblemScraper } from './platforms/codeforces-problems';
import { GenericProblemScraper } from './platforms/generic-problems';
import type { ScrapedProblem } from './types';
import { ScrapedProblemSchema } from './types';
import { tagProblemTopics } from './classifier';
import type { ClassificationResult, LlmCall } from './classifier';
import { DistributedLock } from './lock';
import { postProcessScrapedProblem } from './post-process';
...
export class ScraperRegistry {
  ...
  async scrapeProblem(
    ...
    progress('Acquiring distributed lock...');
    const lock = new DistributedLock(url, 30_000);
    const lockResult = await lock.acquire();

    if (!lockResult) {
      // Couldn't acquire lock. Wait for it to release, then check cache again.
      progress('Another worker is scraping this problem. Waiting...');
      await DistributedLock.waitForLockRelease(url, 35_000);
      
      if (this.cache) {
        try {
          progress('Checking DB cache again after waiting...');
          const cached = await this.cache.findByUrl(url, ttl);
          if (cached) {
            return { success: true, problem: cached, fromCache: true };
          }
      ...
      return {
        ...
        reason: 'SCRAPE_ERROR',
        requiresManualPaste: false,
        error: 'FencingTokenLostError: Lock was released but data not found in cache.',
        url,
      };
    }

    try {
      lockResult.startHeartbeat();

      // ── 3. Find the right scraper ─────────────────────────────────────────────
      ...
      // ── 4. Scrape with 1 retry on transient errors ────────────────────────────
      for (let attempt = 1; attempt <= 2; attempt++) {
        const result = await scraper.scrapeProblem(url, opts);

        if (result.problem) {
          try {
            // Apply deterministic post-processing to ALL scraper paths.
            // This fixes HTML entities, strips iframes, normalizes headers,
            // and strips duplicate examples — regardless of which scraper ran.
            const cleanedProblem = postProcessScrapedProblem(result.problem);
            // Validate through the strict Zod schema boundary
            const validatedProblem = ScrapedProblemSchema.parse(cleanedProblem);
            return { 
              success: true, 
  ...
  async scrapeProblemAndClassify(
    ...
    const statement =
      scrapeResult.problem.content?.problemStatementMarkdown ??
      (scrapeResult.problem as any).statement ??
      (scrapeResult.problem as any).description ??
      '';
